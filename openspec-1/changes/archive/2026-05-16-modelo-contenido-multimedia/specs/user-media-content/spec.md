...
La acción de publicación (p. ej. `PATCH` con `distributionStatus: testing` o `published: true` según contrato) SHALL establecer `moderation_status=approved` en MVP automático tras validación de asset, y SHALL fijar `published_at` cuando pase a estado visible.
