...
`POST /open-requests/interactions` SHALL be reachable without authentication.
...
The client SHALL schedule DOM-dependent telemetry only after the list is rendered, using injection-context-safe APIs (`runInInjectionContext` + `afterNextRender`). MUST NOT throw `NG0203` nor block list load.
