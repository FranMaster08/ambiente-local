...
Ranking del feed de reels UGC y listados derivados (Home destacados, `/feed/reels`): score on-read, columna `ranking_score` para materialización futura, fases testing→scaling y personalización mínima.
...
- **THEN** no aparece en `GET /feed/reels` ni en `GET /home/featured-reels` hasta el día siguiente
...
- **WHEN** `GET /feed/reels?anonymousId=<id>` o `GET /home/featured-reels?anonymousId=<id>` y el actor ya vio un reel
